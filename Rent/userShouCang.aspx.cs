...
namespace Rent
{
    public partial class userShouCang : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    string sql = "select * from house where houseID in(select likehouse.HouseID from likehouse where likehouse.UserID='"+Users.userID+"')";
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        linklike.Text = "取消收藏";
                        linklike.NavigateUrl = "userShouCangLianJie.aspx?likeHouseID=" + dt.Rows[i].ItemArray[0].ToString();
