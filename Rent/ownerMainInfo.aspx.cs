...
namespace Rent
{
    public partial class ownerMainInfo : System.Web.UI.Page
    {
        protected static int number = 2;
        public void Ownerhouse(string sql)
        {
        ...
        public void Owner(string sql)
        {
            ...
            Image ownerImage = new Image();
            ownerImage.ImageUrl = dt.Rows[0].ItemArray[4].ToString();
            ownerImage.Width = 124;
            ownerImage.Height = 165;

            Label lownerTel = new Label();
            lownerTel.Text ="房东电话："+ dt.Rows[0].ItemArray[2].ToString();

            Label lownerName = new Label();
            lownerName.Text = "房东姓名："+dt.Rows[0].ItemArray[1].ToString();

            main1.Style.Add("width", "500px");
            main1.Style.Add("height", "165px");
            image1.Controls.Add(ownerImage);
            image1.Style.Add("width", "124px");
            image1.Style.Add("height", "165px");
            image1.Style.Add("float", "left");
            ownerName1.Controls.Add(lownerName);
            ownerName1.Style.Add("padding-bottom", "25px");
            ownerPhone1.Controls.Add(lownerTel);
            ownerPhone1.Style.Add("float", "left");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                string sql = "select * from owner where ownerID='" + Request.QueryString["ownerID"].ToString() + "'";
                Owner(sql);
                //string sql1 = "";
            }
        }
        ...
        {
            string sql1 = "select * from owner where ownerID='" + Request.QueryString["ownerID"].ToString() + "'";
            if (number % 2 == 0)
            {
                Label1.Visible = true;
                string sql = "select * from house where houseState='未出租' and ownerID='" + Request.QueryString["ownerID"].ToString() + "'";
                Ownerhouse(sql);
            }
            else
            {
